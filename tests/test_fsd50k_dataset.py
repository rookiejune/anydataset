...
from anydataset.datasets.fsd50k import FSD50KDataset, fsd50k_spec


class FSD50KDatasetTest(unittest.TestCase):
    def test_prepare_caches_split_file_list(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch(
                "anydataset.datasets.fsd50k.dataset._list_fsd50k_files",
    ...
    def test_iter_indexed_samples_downloads_only_selected_shard(self):
        ...
        with mock.patch.dict("sys.modules", {"huggingface_hub": fake_hub}):
            with mock.patch(
                "anydataset.datasets.fsd50k.dataset._load_audio",
                return_value=([0.1, 0.2], 44100),
            ):
                rows = list(
                    FSD50KDataset().iter_indexed_samples(
                        manifest,
                        num_shards=2,
                        shard_id=1,
                    )
    ...
    def test_rejects_unknown_split(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaises(ValueError):
                FSD50KDataset().prepare(spec, cache)
