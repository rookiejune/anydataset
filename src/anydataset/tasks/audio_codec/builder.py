...
from anydataset.tasks.audio_codec.schema import (
    SAMPLE_RATE_KEY,
    TEXT_KEY,
    WAVEFORM_KEY,
)
...
class AudioCodecFormatter(SampleFormatter):
    ...
    def __call__(self, sample: Sample) -> Sample:
        waveform_value, source_sample_rate = self._extract_waveform(sample.data)
        ...
        data[WAVEFORM_KEY] = waveform.contiguous()
    ...
    def _extract_waveform(self, row: Mapping[str, Any]) -> tuple[Any, int | None]:
        if self.waveform_key not in row:
            raise KeyError(f"Audio codec samples must include `{self.waveform_key}`.")
        return row[self.waveform_key], _maybe_int(row.get(self.sample_rate_key))

    def _extract_text(self, row: Mapping[str, Any]) -> str | None:
        if self.text_key in row and row[self.text_key] is not None:
            return str(row[self.text_key])
        return None
    ...
    def _resample(self, waveform: torch.Tensor, source_sample_rate: int) -> torch.Tensor:
        ...
        try:
            ...
            return _linear_resample(waveform, source_sample_rate, self.sample_rate)

        return audio_functional.resample(waveform, source_sample_rate, self.sample_rate)
...
def _linear_resample(
    ...
    target_frames = max(
        1,
        round(waveform.shape[-1] * target_sample_rate / source_sample_rate),
    )
