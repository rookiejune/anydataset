...
factory 标识会纳入函数代码、默认参数和 closure 值。调用方还可以设置 `input_id` 和
`provider_id`，为可调用对象无法表达的输入快照或模型 checkpoint 提供显式语义版本；
...
`filter/part-xxxxx.log` 和 `materializer/part-xxxxx.log`。两者的 rank 0 都把 worker
生命周期写到 stderr；filter 额外报告 scan/writer 进度，materializer 报告
