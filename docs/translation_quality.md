...
- `clean`：当前轻量规则均未命中；它适合做核心训练集候选，但不表示规则已经证明
  语义翻译正确。
- `usable`：当前规则确认简单数值相等，但表面格式不同；神经后端也可以把样本上限
  限制到该等级。
...
5. **软结构异常**：中等程度的长度比例或文字脚本偏差输出 `review`；极端偏差仍然
   `reject`。
6. **未命中规则**：输出 `clean`，但这只表示轻量规则没有发现问题，不是语义证明。
...
其他轻量规则未命中而得到 `clean`。这些约束应由调用方增加专用规则或交给后端复核，
...
2. 源文和译文长度比例异常。
3. 源文和译文在不同语言时完全相同。
4. 简单数字 token、占位符和 HTML 标签的一致性；复杂数字表达统一进入 `review`。
...
神经网络分数不能直接把样本升回 `clean`。当前轻量规则优先处理：
...
简单数值不同默认 `reject`；表面形式不同默认 `usable`。Bicleaner 分数很高时，
